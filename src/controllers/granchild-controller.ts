...
router.post("/new", verifyToken, (req: Request, res: Response) => {
  ...
  Promise.all(
    grandChildren.map((grandChild: GrandChild) => {
      grandChild.session_id = session_id;
      grandChild.grandchild_id = uuid();
      grandChild = ObjectUtils.mapEmptyStrings(grandChild);
      const params: UpsertOptions<GrandChild> = new UpsertOptions(
        "grandchildren",
        grandChild,
      );
      return dynamoService.upsert(params).then((resp: boolean) => {
        if (resp) {
          return true;
        }
      });
    ...
    .then(() => {
      res.send({
        ...
        message: `Successfully inserted grand children into session: ${JSON.stringify(
          session_id,
        )}`,
      });
    })
